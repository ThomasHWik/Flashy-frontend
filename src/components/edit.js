...
function Edit() {
  ...
  function checkAuthorization(deckusername) {
    return (
      ...
      localStorage.getItem("flashyUserName") === deckusername
    );
  }
  ...
  async function fetchFlashyInfo() {
    ...
    if (result.status === 200) {
      ...
      alert(
        "Failed to fetch flashcard / You are not authorized to edit this flashcard."
      );
  ...
  return (
    <div className="editcontainer">
      ...
      <div className="editmaincontainer">
        <h1>
          Edit Flashcard -{" "}
          <span style={{ fontWeight: "bold" }}>{initialDeck.name}</span>
        </h1>
        <span>
          <h3>Made by: {initialDeck.username}</h3>
        </span>
          ...
          <div className="editheadercontainer">
            <div>
              ...
              {checkAuthorization(initialDeck.username) ? (
                <div className="editSwitchContainer">
                  <div>
                    <p>Set private</p>
            ...
            <div className="edit_currenttags_container">
              <div>
                {tags.map((tag) => {
                  return (
                    <div className="edit_chosentag" key={tag}>
                      <IoIosRemoveCircle
                        className="edit_removetagicon"
                        color="#bb1818"
                        onClick={() =>
        ...
        <div className="editsectioncontainer">
          ...
          <div className="confirmeditcontainer">
            <button disabled={disableConfirm} onClick={() => confirmEdit()}>
              Confirm edit
            </button>
          </div>
        </div>
      ...
      <div className="box">
        ...
        <div className="question">
          <textarea
            ...
            onInput={(e) => updateQuestion(e)}
            value={question}
          />
          <div className="edit_boximagecontainer">
            <div
              className="edit_boximagepopup"
              style={{ display: showQImage ? "flex" : "none" }}
            >
              <img
                src={
                  uploadedQFile
                    ? URL.createObjectURL(uploadedQFile)
                    : oldQImage
                    ? oldQImage
                    : ""
                }
            ...
            {uploadedQFile ? (
              <CiImageOn
                onMouseEnter={() => setShowQImage(true)}
                onMouseLeave={() => setShowQImage(false)}
                size={25}
                className="edit_boximageicon"
              />
            ) : null}
            {!uploadedQFile && oldQImage ? (
              <CiImageOn
                onMouseEnter={() => setShowQImage(true)}
                onMouseLeave={() => setShowQImage(false)}
                size={25}
            ...
            <input
              ...
              style={{ display: "none" }}
              ref={qImageInputRef}
              className="qImageInput"
              onChange={handleQFileUpload}
            />
            <button onClick={() => qImageInputRef.current.click()}>
              Choose image
            </button>

            {uploadedQFile || oldQImage ? (
              <button className="edit_removefilebtn" onClick={removeQFile}>
                Remove image
              </button>
            ) : null}
          </div>
        ...
        <div className="answer">
          ...
          <div className="edit_boximagecontainer">
            <div
              className="edit_boximagepopup"
              style={{ display: showAImage ? "flex" : "none" }}
            >
              <img
                src={
                  uploadedAFile
                    ? URL.createObjectURL(uploadedAFile)
                    : props.oldaimage
                    ? oldAImage
                    : ""
                }
            ...
            {uploadedAFile ? (
              <CiImageOn
                onMouseEnter={() => setShowAImage(true)}
                onMouseLeave={() => setShowAImage(false)}
                size={25}
                className="edit_boximageicon"
              />
            ) : null}
            {!uploadedAFile && oldAImage ? (
              <CiImageOn
                onMouseEnter={() => setShowAImage(true)}
                onMouseLeave={() => setShowAImage(false)}
                size={25}
            ...
            <input
              ...
              style={{ display: "none" }}
              ref={aImageInputRef}
              className="aImageInput"
              onChange={handleAFileUpload}
            />
            <button onClick={() => aImageInputRef.current.click()}>
              Choose image
            </button>
            {uploadedAFile || oldAImage ? (
              <button className="edit_removefilebtn" onClick={removeAFile}>
                Remove image
              </button>
            ) : null}
          </div>
